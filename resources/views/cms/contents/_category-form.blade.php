...
<div class="grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
    <section class="cms-card cms-gradient-card space-y-5 p-6">
        <div>
            ...
            <select id="menu_item_id" name="menu_item_id" class="cms-input mt-2">
                ...
                @foreach (($menuItemOptions ?? collect()) as $menuItemOption)
                    <option value="{{ $menuItemOption->id }}" @selected((string) old('menu_item_id', $content->menu_item_id) === (string) $menuItemOption->id)>{{ $menuItemOption->title }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input id="name" name="name" type="text" value="{{ old('name', $content->name) }}" class="cms-input mt-2" required>
        </div>
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="5" class="cms-textarea mt-2">{{ old('description', $content->description) }}</textarea>
        </div>
    ...
    <aside class="cms-card cms-gradient-card space-y-5 p-6">
        <div>
            ...
            <input id="sort_order" name="sort_order" type="number" min="0" value="{{ old('sort_order', $content->sort_order ?? 0) }}" class="cms-input mt-2">
        </div>
        ...
        <label class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 text-sm text-slate-700 dark:border-white/10 dark:bg-slate-950/30 dark:text-stone-200">
            <input type="checkbox" name="is_active" value="1" class="h-4 w-4 rounded border-slate-300 bg-white text-sky-500 focus:ring-sky-400 dark:border-white/20 dark:bg-slate-950" @checked(old('is_active', $content->is_active ?? true))>
            Active category
        </label>
